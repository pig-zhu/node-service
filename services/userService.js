...
function activity(req, res, next) {
  ...
  if (!err.isEmpty()) {
    ...
    let { id } = req.body;
    const queryAll = "select sys_message.content,sys_message.create_time,sys_user.nickname from sys_message,sys_user WHERE sys_message.create_user = sys_user.id";
    querySql(queryAll).then(data => {
      if (!data || data.length === 0) {
        res.json({
          code: CODE_ERROR,
